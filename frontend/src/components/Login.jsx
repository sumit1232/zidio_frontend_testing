...
const Login = ({ setUser }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center relative text-white font-sans">
      <img src={randomImage} alt="bg" className="absolute inset-0 w-full h-full object-cover opacity-25 z-0" />
      <div className="z-10 w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 p-8 rounded-2xl shadow-2xl">
        <h2 className="text-4xl font-bold text-center mb-6 tracking-wide text-orange-400">Hero Login</h2>
        {message && (
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit"
            className="w-full bg-orange-400 hover:bg-orange-500 text-black font-bold py-3 rounded-md transition-all">
            Enter Universe
          </button>
        </form>
        <p className="text-center mt-4">
          Not a hero yet? <Link to="/signup" className="text-orange-300 font-semibold hover:underline">Join Now</Link>
        </p>
      </div>
    </div>
  );
